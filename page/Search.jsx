...
export default function Search({ navigation }) {
  ...
  const handleDistrictChange = (itemValue) => {
    setSearch({city: selectedCity, district : itemValue});
    navigation.goBack();
  };
  return (
    <View style={{ paddingHorizontal: 20, paddingTop: 20 }}>
      <View style={{ height: open ? 300 : "auto"}}>
        <DropDownPicker
          open={open}
      ...
      {selectedCity && (
        <DropDownPicker
          open={open2}
          items={cities.find(city => city.name === selectedCity).districts.map(district => ({ label: district, value: district }))}
          setOpen={setOpen2}
          placeholder="구(군) 선택"
